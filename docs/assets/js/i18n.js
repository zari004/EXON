...
(function () {
  ...
  add('Biz haqimizda', 'О нас', 'About us');
  add('Blog', 'Блог', 'Blog');
  ...
  add('Oldin boshlaganlar allaqachon yutmoqda. Bugun boshlaganlar esa hali ularga yetib olish imkoniga ega.', 'Те, кто начал раньше, уже выигрывают. У тех, кто начинает сегодня, ещё есть шанс их догнать.', 'Those who started earlier are already winning. Those who start today still have a chance to catch up.');
  ...
  /* Biz haqimizda */
  add('Biz haqimizda — EXON marketpleys ekspertlari', 'О нас — эксперты EXON по маркетплейсам', 'About us — EXON marketplace experts');
  add("Marketpleysni boshqariladigan biznesga aylantiramiz.", 'Превращаем маркетплейс в управляемый бизнес.', 'We turn marketplaces into a manageable business.');
  add("EXON — Uzum va Yandex Marketda strategiya, ishga tushirish, boshqaruv, tizim va o'sishni bitta yo'nalishga birlashtiradigan professional jamoa.", 'EXON — профессиональная команда, которая объединяет стратегию, запуск, управление, систему и рост на Uzum и Yandex Market.', 'EXON is a professional team that unites strategy, launch, management, systems and growth on Uzum and Yandex Market.');
  add('Biz bilan ishlash', 'Работать с нами', 'Work with us');
  add("Jarayonni ko'ring", 'Посмотреть процесс', 'See our process');
  add("Bozor ko'rsatkichi", 'Показатель рынка', 'Market indicator');
  add("15,2 trln so'm", '15,2 трлн сум', 'UZS 15.2 trillion');
  add("2024-yilda O'zbekistondagi elektron tijorat hajmi. Bu 2020-yildagidan 15 baravar ko'p.", 'Объём электронной торговли в Узбекистане в 2024 году. Это в 15 раз больше, чем в 2020 году.', 'Uzbekistan’s e-commerce volume in 2024 — 15 times higher than in 2020.');
  add("Manba: Milliy statistika qo'mitasi", 'Источник: Национальный комитет по статистике', 'Source: National Statistics Committee');
  add('Biz kimimiz?', 'Кто мы?', 'Who are we?');
  add("Bir xodim emas — biznesingizga mas'ul butun jamoa.", 'Не один сотрудник, а целая команда, отвечающая за ваш бизнес.', 'Not one specialist, but a whole team accountable for your business.');
  add("Marketpleysdagi o'sish bitta reklama yoki chiroyli kartochkadan iborat emas. EXON barcha muhim yo'nalishlarni yagona strategiya ostida birlashtiradi va har bir qarorni raqamlar bilan boshqaradi.", 'Рост на маркетплейсе — это не одна реклама и не красивая карточка. EXON объединяет все ключевые направления одной стратегией и принимает решения на основе цифр.', 'Marketplace growth is more than one ad or a beautiful product card. EXON unites every key function under one strategy and manages decisions with data.');
  add("Bozor, raqobat va mahsulot iqtisodini tahlil qilib, aniq yo'l xaritasini tuzamiz.", 'Анализируем рынок, конкурентов и экономику продукта, затем создаём чёткую дорожную карту.', 'We analyze the market, competition and product economics, then build a clear roadmap.');
  add('Kartochka va kontent', 'Карточки и контент', 'Product cards and content');
  add("Mahsulotni tushunarli, ishonchli va sotuvga tayyor ko'rinishga olib kelamiz.", 'Делаем продукт понятным, убедительным и готовым к продажам.', 'We make the product clear, credible and ready to sell.');
  add('Reklama va analitika', 'Реклама и аналитика', 'Advertising and analytics');
  add("Trafikni shunchaki oshirmaymiz — xarajat, konversiya va foydani birga kuzatamiz.", 'Не просто наращиваем трафик — вместе отслеживаем затраты, конверсию и прибыль.', 'We do more than grow traffic — we track cost, conversion and profit together.');
  add('Operatsion boshqaruv', 'Операционное управление', 'Operational management');
  add('Narx, qoldiq, logistika va kundalik vazifalarni tartibli tizimda boshqaramiz.', 'Управляем ценами, остатками, логистикой и ежедневными задачами в единой системе.', 'We manage pricing, stock, logistics and daily tasks through one structured system.');
  add('Biz nimaga ishonamiz?', 'Во что мы верим?', 'What do we believe?');
  add("O'sish faqat", 'Рост возможен только при наличии', 'Growth only happens with');
  add('strategiya va tizim', 'стратегии и системы', 'strategy and systems');
  add("bor joyda paydo bo'ladi.", '.', '.');
  add("Mijozlar EXONni uch so'z bilan ta'riflaydi: natija, tartib va professionallik.", 'Клиенты описывают EXON тремя словами: результат, порядок и профессионализм.', 'Clients describe EXON in three words: results, order and professionalism.');
  add('Sizga beradigan foydamiz', 'Что вы получаете', 'What you gain');
  add("Ko'proq nazorat. Kamroq tartibsizlik. Barqarorroq o'sish.", 'Больше контроля. Меньше хаоса. Более устойчивый рост.', 'More control. Less chaos. More sustainable growth.');
  add('FOKUS', 'ФОКУС', 'FOCUS');
  add('Egaga biznes uchun vaqt', 'Время владельца для бизнеса', 'More time for the business owner');
  add('Kundalik mayda vazifalarni jamoamizga topshirasiz va mahsulot, moliya hamda rivojlanishga fokus qilasiz.', 'Вы передаёте ежедневные задачи нашей команде и фокусируетесь на продукте, финансах и развитии.', 'You hand daily tasks to our team and focus on product, finance and development.');
  add('NAZORAT', 'КОНТРОЛЬ', 'CONTROL');
  add("Raqamlarda ko'rinadigan holat", 'Прозрачная картина в цифрах', 'A clear view in numbers');
  add('Trafik, reklama, konversiya, qoldiq va natijalar bitta boshqaruv mantiqida kuzatiladi.', 'Трафик, реклама, конверсия, остатки и результаты отслеживаются в единой системе управления.', 'Traffic, ads, conversion, stock and results are tracked in one management framework.');
  add("O'SISH", 'РОСТ', 'GROWTH');
  ...
  add('EXON ishlash bosqichlari', 'Этапы работы EXON', 'EXON workflow stages');
  add('Qadriyatlarimiz', 'Наши ценности', 'Our values');
  add('Natija ishonchdan, ishonch esa standartdan boshlanadi.', 'Результат начинается с доверия, а доверие — со стандарта.', 'Results begin with trust, and trust begins with standards.');
  add("Mas'uliyat", 'Ответственность', 'Accountability');
  add("Ishning mezoni — bajarilgan vazifa emas, o'lchanadigan natija.", 'Мерило работы — не выполненная задача, а измеримый результат.', 'The measure of work is not a completed task, but a measurable result.');
  add('Halollik', 'Честность', 'Honesty');
  add("Muammoni yashirmaymiz. Uni tez ko'rsatamiz, tushuntiramiz va hal qilamiz.", 'Мы не скрываем проблемы. Быстро показываем, объясняем и решаем их.', 'We do not hide problems. We surface, explain and solve them quickly.');
  add('Intizom', 'Дисциплина', 'Discipline');
  ...
  add("Har bir loyiha uchun aniq jarayon, mas'ul va o'lchanadigan natijalarni yanada kuchaytirish.", 'Усиливать чёткие процессы, ответственность и измеримые результаты в каждом проекте.', 'Strengthen clear processes, accountability and measurable results in every project.');
  add("Ma'lumotga asoslangan qarorlar", 'Решения на основе данных', 'Data-driven decisions');
  add("Reklama va savdoni his-tuyg'u bilan emas, tez va tushunarli analitika bilan boshqarish.", 'Управлять рекламой и продажами не интуицией, а быстрой и понятной аналитикой.', 'Manage ads and sales with fast, clear analytics rather than instinct.');
  add('Jamoa ekspertizasiga sarmoya', 'Инвестиции в экспертизу команды', 'Invest in team expertise');
  add("Har bir yo'nalishda kuchli mutaxassislarni rivojlantirib, mijozga yaxlit natija berish.", 'Развивать сильных специалистов в каждом направлении и давать клиенту целостный результат.', 'Develop strong specialists in every function and deliver one integrated result.');
  add('Kimlar uchun?', 'Для кого?', 'Who is it for?');
  add("O'sishga tayyor bizneslar uchun.", 'Для бизнеса, готового к росту.', 'For businesses ready to grow.');
  add('Amaldagi mahsuloti va savdosi bor brendlar, importyorlar va ishlab chiqaruvchilar.', 'Для брендов, импортёров и производителей с действующим продуктом и продажами.', 'For brands, importers and manufacturers with an active product and sales.');
  add("Marketpleysni qisqa aksiya emas, 3–6 oylik investitsiya deb ko'radigan jamoalar.", 'Для команд, которые воспринимают маркетплейс как инвестицию на 3–6 месяцев, а не короткую акцию.', 'For teams that see marketplaces as a 3–6 month investment, not a short campaign.');
  add('Tovar, logistika va marketingni bir tizimda rivojlantirishga tayyor bizneslar.', 'Для бизнеса, готового развивать товар, логистику и маркетинг в одной системе.', 'For businesses ready to develop product, logistics and marketing as one system.');
  add('Keyingi qadam', 'Следующий шаг', 'Next step');
  add("Marketpleysdagi o'sishingizni tizimdan boshlaymiz.", 'Начнём ваш рост на маркетплейсе с системы.', 'Let’s start your marketplace growth with a system.');
  add("Vaziyatingizni tushunamiz, o'sish nuqtalarini aniqlaymiz va birinchi aniq qadamni tavsiya qilamiz.", 'Разберём вашу ситуацию, найдём точки роста и предложим первый конкретный шаг.', 'We will understand your situation, identify growth opportunities and recommend the first clear step.');
